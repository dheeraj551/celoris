-- Social Media Tables Schema for InstaLinkr+ - NO FK VERSION
-- Run this in your Supabase SQL Editor
-- This version doesn't reference auth.users initially to avoid FK issues

-- Step 1: Create the social_profiles table (NO foreign key to auth.users yet)
CREATE TABLE IF NOT EXISTS social_profiles (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID, -- We'll link this to auth.users later
...
-- Step 2: Enable RLS on social_profiles
ALTER TABLE social_profiles ENABLE ROW LEVEL SECURITY;

-- Step 3: Create social_profiles policies
...
-- Step 5: Enable RLS on swipes
ALTER TABLE swipes ENABLE ROW LEVEL SECURITY;

-- Step 6: Create swipes policies
DROP POLICY IF EXISTS "Users can view own swipes" ON swipes;
CREATE POLICY "Users can view own swipes" ON swipes
  FOR SELECT USING (auth.uid() IN (SELECT user_id FROM social_profiles WHERE id = swiper_id));

DROP POLICY IF EXISTS "Users can create swipes" ON swipes;
CREATE POLICY "Users can create swipes" ON swipes
  FOR INSERT WITH CHECK (auth.uid() IN (SELECT user_id FROM social_profiles WHERE id = swiper_id));
...
-- Step 8: Enable RLS on matches
ALTER TABLE matches ENABLE ROW LEVEL SECURITY;

-- Step 9: Create matches policies
...
-- Step 11: Enable RLS on messages
ALTER TABLE messages ENABLE ROW LEVEL SECURITY;

-- Step 12: Create messages policies
...
CREATE POLICY "Users can send messages" ON messages
  FOR INSERT WITH CHECK (
    ...
    AND auth.uid() IN (SELECT user_id FROM social_profiles WHERE id = sender_id)
  );

-- Step 13: Create creator_earnings table
...
-- Step 14: Enable RLS on creator_earnings
ALTER TABLE creator_earnings ENABLE ROW LEVEL SECURITY;

-- Step 15: Create creator_earnings policies
...
CREATE POLICY "Users can view own earnings" ON creator_earnings
  FOR SELECT USING (
    auth.uid() IN (
      SELECT user_id FROM social_profiles 
      WHERE id IN (creator_id, supporter_id)
    )
  );

-- Step 16: Create content_monetization table
...
-- Step 17: Enable RLS on content_monetization
ALTER TABLE content_monetization ENABLE ROW LEVEL SECURITY;

-- Step 18: Create content_monetization policies
...
-- Step 19: Create creator_subscriptions table
...
-- Step 20: Enable RLS on creator_subscriptions
ALTER TABLE creator_subscriptions ENABLE ROW LEVEL SECURITY;

-- Step 21: Create creator_subscriptions policies
...
-- Step 22: Create instagram_posts table
...
-- Step 23: Enable RLS on instagram_posts
ALTER TABLE instagram_posts ENABLE ROW LEVEL SECURITY;

-- Step 24: Create instagram_posts policies
...
-- Step 25: Create live_streams table
...
-- Step 26: Enable RLS on live_streams
ALTER TABLE live_streams ENABLE ROW LEVEL SECURITY;

-- Step 27: Create live_streams policies
...
-- Step 28: Create functions and triggers
...
-- Step 29: Create triggers for updated_at
...
-- Step 30: Create match creation function and trigger
...
-- Step 31: Create trigger for automatic match creation
...
-- Step 33: Insert sample Instagram posts
