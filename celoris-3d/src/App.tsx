...
import { generate3DModelMock } from './services/mock3DService';
...
export default function App() {
  ...
  const applyState = (state: GenerationState) => {
    ...
    // Note: We don't restore imageFile, but handleGenerate uses imagePreview if imageFile is null
  };
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-50 font-sans selection:bg-indigo-500/30 flex flex-col">
      ...
      <header className="h-16 border-b border-white/5 flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-lg flex items-center justify-center shadow-[0_0_15px_rgba(168,85,247,0.5)]">
            <Orbit className="w-4 h-4 text-white" />
          </div>
          <h1 className="font-semibold text-lg tracking-tight">Celoris 3D</h1>
          <span className="ml-2 px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider bg-white/10 text-neutral-400 rounded-full">
            Beta
          </span>
        </div>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* Sidebar Controls */}
        <aside className="w-full lg:w-80 border-r border-white/5 bg-neutral-900/30 p-6 flex flex-col gap-8 overflow-y-auto shrink-0">
          
          <div className="space-y-4">
            <h2 className="text-sm font-medium text-neutral-200">Generation Mode</h2>
            <div className="flex bg-black/50 p-1 rounded-lg border border-white/10">
              <button
                onClick={() => setMode('mesh')}
                className={cn("flex-1 text-xs font-medium py-2 rounded-md transition-all", mode === 'mesh' ? "bg-indigo-500 text-white shadow" : "text-neutral-400 hover:text-neutral-200")}
              >
                3D Mesh (Mock)
              </button>
              <button
                onClick={() => setMode('texture')}
                className={cn("flex-1 text-xs font-medium py-2 rounded-md transition-all", mode === 'texture' ? "bg-indigo-500 text-white shadow" : "text-neutral-400 hover:text-neutral-200")}
              >
                Texture (Gemini)
              </button>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium text-neutral-200">Generation Prompt</h2>
            </div>
            ...
            <div className="relative">
              ...
              {!imagePreview ? (
                <button 
                  ...
                  className="w-full flex flex-col items-center justify-center gap-2 h-24 border-2 border-dashed border-white/10 rounded-xl bg-black/30 hover:bg-white/5 hover:border-white/20 transition-all text-neutral-400"
                >
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Describe a material, texture, or pattern (e.g., 'glowing alien cybernetic metal panels')"
                className="w-full h-32 bg-black/50 border border-white/10 rounded-xl p-4 text-sm text-neutral-200 placeholder:text-neutral-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all resize-none"
              />
          ...
          {mode === 'texture' && (
            <div className="space-y-4">
              <h2 className="text-sm font-medium text-neutral-200">Base Geometry</h2>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      shape === s.id 
                        ...
                        : "bg-black/30 border-white/5 text-neutral-400 hover:bg-white/5 hover:text-neutral-200"
                    )}
                  >
                    {s.icon}
          ...
          <div className="space-y-4">
            <h2 className="text-sm font-medium text-neutral-200">Material Colors</h2>
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2 bg-black/30 border border-white/5 rounded-lg p-2">
                  ...
                  />
                  <span className="text-xs font-mono text-neutral-300 uppercase">{baseColor}</span>
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-xs text-neutral-400">Emissive</label>
                <div className="flex items-center gap-2 bg-black/30 border border-white/5 rounded-lg p-2">
                  <input 
                    type="color" 
                    value={emissiveColor} 
                    ...
                    className="w-6 h-6 rounded cursor-pointer bg-transparent border-0 p-0"
                  />
                  <span className="text-xs font-mono text-neutral-300 uppercase">{emissiveColor}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2 max-h-40 overflow-y-auto pr-1">
                {[...history].reverse().map((state, revIndex) => {
                  ...
                  if (index === 0) return null; // Skip initial empty state
                  return (
                    <button
                      key={index}
                      onClick={() => {
                    ...
                    >
                      <div className="font-medium truncate mb-1.5" title={state.prompt}>
                        {state.prompt || "Image reference generation"}
                      </div>
                      <div className="flex items-center gap-2 text-[10px] opacity-60 font-mono uppercase">
          ...
          <div className="mt-auto pt-6 border-t border-white/5">
            <button
              ...
              className="w-full relative group overflow-hidden rounded-xl bg-indigo-600 px-4 py-3 font-medium text-white transition-all hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <div className="absolute inset-0 flex items-center justify-center gap-2">
                {isGenerating ? (
                  ...
                  <>
                    ...
                    <span>Generate {mode === 'texture' ? '3D Material' : '3D Model'}</span>
                  </>
                )}
              </div>
            </button>
        ...
        <section className="flex-1 p-6 relative flex flex-col min-h-[500px]">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-medium text-neutral-400">Viewport 3D</h2>
            <div className="flex items-center gap-2">
              <button 
                onClick={handleUndo} 
                disabled={historyIndex <= 0 || isGenerating}
                className="p-1.5 text-neutral-400 hover:text-white disabled:opacity-30 disabled:hover:text-neutral-400 transition-colors rounded-md hover:bg-white/5"
                title="Undo"
              >
              ...
              <button 
                ...
                className={cn(
                  ...
                  autoRotate ? "text-indigo-400 hover:text-indigo-300" : "text-neutral-400 hover:text-white"
                )}
              ...
              {(textureUrl || modelUrl) && (
                <a 
                  ...
                  className="flex items-center gap-2 text-xs font-medium text-neutral-400 hover:text-white transition-colors bg-white/5 px-3 py-1.5 rounded-lg border border-white/10"
                >
