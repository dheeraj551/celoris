...
import { PLAN_LABEL, getUserPlan } from '@/lib/cafe-class-queue'
...
export async function GET() {
  ...
  try {
    ...
    const [{ data: live }, plan, balance] = await Promise.all([
      admin
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (plan === 'free') {
      return NextResponse.json(
        { error: 'Voice and video calls are for Basic, Pro and Max members.', code: 'needs_plan' },
        { status: 402 }
      )
    }
