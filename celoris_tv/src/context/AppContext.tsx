...
interface AppContextType {
  ...
  uploadTeacherVideo: (data: Partial<Video>) => Video;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [currentVideo, setCurrentVideo] = useState<Video | null>(INITIAL_VIDEOS[0]);
  const [currentPlaylist, setCurrentPlaylist] = useState<Playlist | null>(INITIAL_PLAYLISTS[2]); // My midterms playlist
  const [currentPlaylistIndex, setCurrentPlaylistIndex] = useState<number>(0);
  const [currentView, setCurrentView] = useState<ViewMode>('watch');
  ...
  const uploadTeacherVideo = (data: Partial<Video>): Video => {
    const newVid: Video = {
      id: `vid-${Date.now()}`,
      title: data.title || 'Untitled Lecture',
      description: data.description || 'Lecture video recording and notes.',
      videoUrl:
        data.videoUrl ||
      ...
      tags: data.tags || ['Lecture', 'Education'],
      author: {
        id: currentUser.id,
        name: currentUser.name,
        avatar: currentUser.avatar,
        role: 'professor',
        title: currentUser.bio.split('.')[0] || 'Professor & Researcher',
        institution: currentUser.institution,
        verified: true,
      },
      chapters: data.chapters || [
        { title: 'Lecture Introduction', timestamp: 0 },
        { title: 'Core Theoretical Derivations', timestamp: 300 },
        { title: 'Practical Application & Summary', timestamp: 650 },
      ],
      resources: data.resources || [
        { id: `res-${Date.now()}`, title: 'Lecture Presentation Slides (PDF)', type: 'slides', size: '2.5 MB', url: '#' },
      ],
      transcript: data.transcript || [
        { id: `tr-${Date.now()}`, timestamp: 0, speaker: currentUser.name, text: 'Welcome to this recorded lecture session.' },
      ],
