...
export const TeacherStudioView: React.FC = () => {
  ...
  const [thumbnailUrl, setThumbnailUrl] = useState('https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&auto=format&fit=crop&q=80');
  ...
  const handlePublish = (e: React.FormEvent) => {
    ...
    const newVid = uploadTeacherVideo({
      title: title.trim(),
      ...
      <div className="bg-[#161B16] border border-[#242A24] rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
        ...
        <form onSubmit={handlePublish} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#242A24]">
            <button
              type="submit"
              className="flex items-center gap-2 px-6 py-3 bg-[#7F9172] hover:bg-[#91A582] text-[#0D0F0D] rounded-xl text-xs font-bold shadow-lg shadow-[#7F9172]/20 transition-all hover:scale-102"
            >
              <Upload className="w-4 h-4 text-[#0D0F0D]" /> Publish Lecture to Course
            </button>
          </div>
        </form>
      </div>
