...
-- Final status - FIXED UNION TYPES
SELECT '=== FINAL STATUS SUMMARY ===' as info;
SELECT 
    'Admin users in public.users' as metric,
    (SELECT COUNT(*) FROM public.users WHERE role = 'admin')::text as value
UNION ALL
SELECT 
    'Admin accounts in auth.users' as metric,
    (SELECT COUNT(*) FROM auth.users WHERE email IN ('support@celorisdesigns.com', 'celoris.designs@gmail.com'))::text as value
UNION ALL
SELECT 
    'Total users' as metric,
    (SELECT COUNT(*) FROM public.users)::text as value
UNION ALL
SELECT 
