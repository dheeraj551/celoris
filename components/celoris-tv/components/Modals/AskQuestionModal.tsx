...
import { X, Clock, Code, MessageSquarePlus } from 'lucide-react';
...
export const AskQuestionModal: React.FC<Props> = ({
  ...
  const { addQuestion, videoCurrentTime } = useApp();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="bg-[#0e121e]/95 backdrop-blur-2xl border border-white/10 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative text-slate-200 max-h-[90vh] overflow-y-auto custom-scrollbar">
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-3.5 bg-black/60 border border-white/10 rounded-2xl flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {useTimestamp && (
                <span className="px-2 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-xs font-semibold rounded-lg">
              ...
              <input
                ...
                className="w-4 h-4 rounded text-emerald-500 focus:ring-emerald-500/30 border-white/20 bg-black/60 cursor-pointer accent-emerald-500"
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
              Question Summary <span className="text-emerald-400">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
              Detailed Context / What have you tried? <span className="text-emerald-400">*</span>
            </label>
            <textarea
              rows={4}
              required
          ...
          <div>
            ...
            {showCodeInput && (
              <textarea
                ...
                className="w-full p-3 font-mono text-xs bg-black/80 border border-white/10 rounded-xl text-emerald-300 placeholder-slate-500 focus:outline-hidden focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all"
              />
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {tags.map(t => (
                <span
                  key={t}
                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-300"
                >
                  #{t}
                  <button
                    type="button"
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-1.5 bg-black/60 border border-white/10 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all"
              />
              <button
                type="button"
                onClick={handleAddTag}
                className="px-3.5 py-1.5 bg-white/5 hover:bg-white/10 text-xs font-semibold text-slate-200 rounded-xl border border-white/10 transition-colors"
              >
                Add Tag
              </button>
            </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/[0.08]">
            ...
            >
              <MessageSquarePlus className="w-4 h-4 text-black stroke-[2.5]" /> Post Question
            </button>
          </div>
        </form>
      </div>
