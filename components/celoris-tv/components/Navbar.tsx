...
export const Navbar: React.FC<Props> = ({ onToggleSidebar }) => {
  ...
  const avatarUrl = profile?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=7F9172&color=fff`;
    ...
    <header className="sticky top-0 z-40 w-full bg-[#121512]/90 backdrop-blur-md border-b border-[#242A24] px-4 lg:px-8 py-3 select-none text-[#E0E5E0] transition-colors">
      <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {/* Quick Action Button — publishing now happens via a pasted
              YouTube link inside Teacher Studio itself, so there's no
              separate "Upload" entry point here. */}
          <button
          ...
          {/* User Profile Avatar Dropdown (real account menu) */}
                ...
                <div className="absolute right-0 mt-2 w-64 bg-[#161B16] border border-[#2A322A] rounded-2xl shadow-2xl p-4 z-50 text-[#E0E5E0] animate-fadeIn">
                  <div className="flex items-center gap-3 pb-3 border-b border-[#2A322A]">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[#95A395] truncate">{user?.email}</p>
                    </div>
                  </div>
                  ...
                  <div className="py-2 space-y-1 text-xs">
                    <button
                      onClick={() => {
                        ...
                        router.push('/social/swipe');
                      }}
                      className="w-full flex items-center gap-2 text-left px-2 py-1.5 rounded-lg hover:bg-[#1E231E] text-[#95A395] hover:text-white transition-colors"
                    >
                    ...
                    <div className="w-full flex items-center gap-2 px-2 py-1.5 text-[#A8B89C]">
                      <Wallet className="w-3.5 h-3.5" />
                      Credits: {profile?.wallet_balance ?? '0'}
                    </div>
                  </div>

                  <div className="pt-2 border-t border-[#2A322A]">
                    <button
                      onClick={() => {
                        setShowProfileMenu(false);
                        handleSignOut();
                      }}
                      className="w-full flex items-center gap-2 text-left px-2 py-1.5 rounded-lg hover:bg-[#1E231E] text-[#C87D55] hover:text-white transition-colors text-xs"
                    >
