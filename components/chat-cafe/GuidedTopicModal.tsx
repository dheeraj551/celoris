...
export const GuidedTopicModal: React.FC<GuidedTopicModalProps> = ({
  ...
  const handleGenerateWithAI = async () => {
    ...
    try {
      const res = await fetch('/api/social/chat-cafe/discussion-prompt', {
        method: 'POST',
      ...
      >
        ...
        <div className="flex items-center gap-2 p-1 bg-black/30 rounded-2xl border border-white/5">
          ...
          >
            ...
            <span>Generate with AI Barista</span>
          </button>
        ...
        {activeTab === 'ai_generate' && (
          <div className="space-y-4 p-4 rounded-2xl bg-black/30 border border-amber-500/20">
            <div className="text-xs text-amber-200/90 leading-relaxed">
              Ask our server-side AI Barista to brew a personalized discussion card tailored to the mood of your table:
            </div>
            ...
            >
              ...
              <span>{isGenerating ? 'Barista AI is brewing a fresh topic...' : 'Generate New Discussion Topic with AI'}</span>
            </button>
          </div>
        )}
