 ...
 */
export function generateTrtcUserSig(userId: string): TrtcJoinCredentials {
  if (!TENCENT_TRTC_SDK_APP_ID || !TENCENT_TRTC_SECRET_KEY) {
  ...
  const userSig: string = api.genSig(userId, USERSIG_EXPIRE_SECONDS);

  return { userSig, sdkAppId: sdkAppIdNum, expiresIn: USERSIG_EXPIRE_SECONDS };
}
