...
export const ModerationPanelModal: React.FC<ModerationPanelModalProps> = ({
  ...
  // AI Moderation Audit via /api/cafe/ai-moderate
  const handleRunAiAudit = async (report: ModerationReport) => {
    setAnalyzingReportId(report.id);
    try {
      const res = await fetch('/api/cafe/ai-moderate', {
        method: 'POST',
                      ...
                      {audit && (
                        <div className="p-2.5 rounded-xl bg-indigo-950/40 border border-indigo-700/40 text-xs text-indigo-200 space-y-1">
                          <div className="font-semibold flex items-center gap-1.5 text-indigo-300">
                            ...
                            <span>Gemini AI Moderation Opinion: Severity {audit.flagSeverity || 'low'}</span>
                          </div>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-emerald-950/30 border border-emerald-700/40 text-xs text-emerald-200 space-y-2">
                ...
                <p className="text-stone-300 leading-relaxed">
                  Our server includes active profanity masking and Gemini-powered moderation assessment. Severe slurs or hostile spam are automatically replaced with café asterisks (<code className="text-amber-300 bg-black/40 px-1 rounded">☕***</code>) and flagged directly to the Reports Queue above for human review.
                </p>
              </div>
